...
class RelationPathItem(BaseModel):
    ...
    direction: RelationDirection = Field(
        ..., description="Direction from path traversal perspective"
    )
...
class ContextArtifact(BaseModel):
    ...
    relevance_score: float | None = Field(
        None, description="Semantic relevance to query (0.0-1.0)"
    )
...
class TemporalHint(BaseModel):
    ...
    days_back: int | None = Field(
        None, description="For 'recent' mode: number of days back"
    )
...
class ContextPreferences(BaseModel):
    ...
    prefer_recent: bool | None = Field(
        None, description="Apply recency bias to scoring"
    )
...
class ContextHints(BaseModel):
    ...
    query: str | None = Field(
        None,
        description="The prompt/question driving this request (enables relevance scoring)",
    )
...
class ContextHintsApplied(BaseModel):
    ...
    token_budget_enforced: bool = Field(
        False, description="Whether token budget was applied"
    )
    temporal_filter_applied: bool = Field(
        False, description="Whether temporal filter was applied"
    )
    relevance_filtering_applied: bool = Field(
        False, description="Whether relevance threshold was used"
    )
    exclusions_applied: int = Field(
        0, description="Number of artifacts excluded by hint"
    )
...
class ContextMetadata(BaseModel):
    ...
    artifact_count: int = Field(
        ..., description="Total artifacts in context (excluding primary)"
    )
    tokens_estimated: int | None = Field(
        None, description="Estimated token count if budget was set"
    )
...
class ContextResponse(BaseModel):
    ...
    artifact: ArtifactResponse = Field(
        ..., description="The primary/requested artifact"
    )
    context: list[ContextArtifact] = Field(
        default_factory=list, description="Related artifacts with relationship metadata"
    )
