"""Artifact service - database operations for artifacts (V2 append-only).
...
async def get_artifact(artifact_id: UUID, tenant_id: int) -> ArtifactResponse | None:
...
async def _get_artifacts_by_ids(tenant_id: int, ids: list[UUID]) -> ArtifactListResponse:
...
async def get_children(artifact_id: UUID, tenant_id: int) -> list[ArtifactResponse]:
    """Get all child artifacts (for positional types)."""
    async with get_connection() as conn:
        result = await conn.execute(
            f"""
            SELECT id, tenant_id, artifact_type, parent_artifact_id,
...
# NOTE: update_artifact and delete_artifact removed - artifacts are append-only
# NOTE: artifact_version functions removed - each artifact is its own identity
