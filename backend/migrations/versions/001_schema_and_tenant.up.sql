...
-- Creates mimirdata schema, vocabulary tables, and tenant table
-- Admin tables are mutable; content tables (later migrations) are append-only
...
COMMENT ON TABLE mimirdata.tenant_type IS 'Vocabulary table for tenant types (admin - mutable)';
...
COMMENT ON TABLE mimirdata.artifact_type IS 'Vocabulary table for artifact types (admin - mutable)';
...
INSERT INTO mimirdata.relation_type (code, display_name, description, inverse_code, is_symmetric, sort_order) VALUES
    ...
    ('supersedes', 'Supersedes', 'Source supersedes/replaces target (editorial intent)', 'superseded_by', false, 50),
...
COMMENT ON TABLE mimirdata.relation_type IS 'Vocabulary table for relation types (admin - mutable)';
COMMENT ON COLUMN mimirdata.relation_type.inverse_code IS 'Code of inverse relation for bidirectional queries';
COMMENT ON COLUMN mimirdata.relation_type.is_symmetric IS 'True if A→B implies B→A (e.g., contradicts, related_to)';

-- =============================================================================
-- TENANT TABLE (Admin - Mutable)
-- =============================================================================
...
COMMENT ON TABLE mimirdata.tenant IS 'Multi-tenant isolation (admin - mutable)';
